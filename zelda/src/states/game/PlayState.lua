...
function PlayState:init()
    ...
    for i = 1, 10 do
        self.dungeon.currentRoom.entities[i].stateMachine = StateMachine {
            ['walk'] = function() return EntityWalkState(self.dungeon.currentRoom.entities[i],self.dungeon) end,
            ['idle'] = function() return EntityIdleState(self.dungeon.currentRoom.entities[i],self.dungeon) end
        }
        self.dungeon.currentRoom.entities[i]:changeState('walk')
    end 
