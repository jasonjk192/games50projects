...
function Room:generateObjects()
    ...
    local types = {'pot_1','pot_2','pot_3'}
    for i = 1, 4 do
        local type = types[math.random(#types)]
        local pot = GameObject(GAME_OBJECT_DEFS[type],
        math.random(MAP_RENDER_OFFSET_X + TILE_SIZE,
        VIRTUAL_WIDTH - TILE_SIZE * 2 - 16),
        math.random(MAP_RENDER_OFFSET_Y + TILE_SIZE,
        VIRTUAL_HEIGHT - (VIRTUAL_HEIGHT - MAP_HEIGHT * TILE_SIZE) + MAP_RENDER_OFFSET_Y - TILE_SIZE - 16))
...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 then
            if math.random(10)==1 and not entity.dead then 
                local heart = GameObject(GAME_OBJECT_DEFS['health'],
                    entity.x,
                    entity.y)
    ...
    for k, projectile in pairs(self.projectiles) do
        ...
        if projectile:wallCollide() then
            projectile.finish = true
        end
        if projectile.finish then
