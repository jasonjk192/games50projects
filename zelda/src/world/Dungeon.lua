...
function Dungeon:beginShifting(shiftX, shiftY)
    ...
    for i = 1, 10 do
        self.nextRoom.entities[i].stateMachine = StateMachine {
            ['walk'] = function() return EntityWalkState(self.nextRoom.entities[i],self) end,
            ['idle'] = function() return EntityIdleState(self.nextRoom.entities[i]) end
        }
        self.nextRoom.entities[i]:changeState('walk')
    end
...
function Dungeon:finishShifting()
    ...
    for i = 1, 10 do
        self.currentRoom.entities[i].stateMachine = StateMachine {
            ['walk'] = function() return EntityWalkState(self.currentRoom.entities[i],self) end,
            ['idle'] = function() return EntityIdleState(self.currentRoom.entities[i]) end
        }
        self.currentRoom.entities[i]:changeState('walk')
    end
end
