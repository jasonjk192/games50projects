            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        local hp,att,def,spd = self.playerPokemon:levelUp()
                        gStateStack:push(BattleMessageState('Congratulations! Level Up!',
                        function()
                            gStateStack:push(BattleMessageState('HP : '..tostring(self.playerPokemon.HP-hp).. ' + '..tostring(hp)
                            ,
                            function()
                            gStateStack:push(BattleMessageState('Attack : '..tostring(self.playerPokemon.attack-att).. ' + '..tostring(att)
                            ,
                            function() 
                                gStateStack:push(BattleMessageState('Defense : '..tostring(self.playerPokemon.defense-def).. ' + '..tostring(def)
                            ,
                            function()
                                gStateStack:push(BattleMessageState('Speed : '..tostring(self.playerPokemon.speed-spd).. ' + '..tostring(spd)
                            ,
                            function()
                                ...
                                menu_items = {
                                    { text = 'HP : '..tostring(self.playerPokemon.HP)},
                                    { text = 'Attack : '..tostring(self.playerPokemon.attack)},
                                    { text = 'Defense : '..tostring(self.playerPokemon.defense)},
                                    { text = 'Speed : '..tostring(self.playerPokemon.speed)},
                                }
                                gStateStack:push(MenuState({state = self,itemSelectable = false, items=menu_items, onFinish = function() Timer.after(1, function()
                                    self:fadeOutWhite()
                                end) end }))
